...
- **Scoring:** `calcPts(pred, m)` returns **penalty points** (lower = better). It adds: wrong outcome (+2 if a draw was involved, +3 otherwise) + absolute goal error per team (`|score_a - pred_a| + |score_b - pred_b|`). An exact score = 0 pts. `isExact(pred, m)` returns true when both scores match exactly. The leaderboard sorts ascending. Do not duplicate this logic elsewhere.
